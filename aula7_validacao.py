...
# Validação manual e explícita das regras de negócio (aula 07)
...
# Coleção de 5 produtos em memória (aula 7)
produtos = [
    ...
    for index, item in enumerate(produtos):
        if item["id"] == id:
            erro_validacao = validar_produto(
                produto.nome.strip() if produto.nome is not None else None,
                produto.preco,
            )
            if erro_validacao:
...
# Rodar servidor:
# uvicorn aula7_validacao:app --reload
